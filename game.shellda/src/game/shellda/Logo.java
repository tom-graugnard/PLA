...
public class Logo {
	...
	Logo(Model model, int no, BufferedImage sprite, int rows, int columns, int x, int y, float scale) {
		m_model = model;
		m_sprite = sprite;
		m_ncols = columns;
		m_nrows = rows;
	...
	boolean inside(float x, float y) {
		float scale_mw = m_w * Options.LogoScale;
		float scale_mh = m_h * Options.LogoScale;
